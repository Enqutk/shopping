...
export default function ProductDetailPage() {
    ...
    return (
      <div className="store-page">
        ...
        <div className="flex flex-1 flex-col items-center justify-center px-4">
          ...
          <Link href="/products" className="mt-4 text-sm uppercase tracking-widest text-luxe-accent hover:underline">
            ← Back to catalog
          </Link>
        </div>
        <StoreFooter />
  ...
  return (
    <div className="store-page">
      <StoreScene>
      <StoreHeader />
      <main id="main-content" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full flex-1">
        <Link
          href="/products"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 items-start">
          ...
          <div className="flex flex-col lg:sticky lg:top-28" style={{ transform: 'translateZ(32px)' }}>
            {product.category && (
            ...
            <div className="mt-8">
              <p className="text-xs font-bold uppercase tracking-widest text-arctic-deep/60 mb-3">Size</p>
              <div className="flex gap-2" role="group" aria-label="Select size">
                {SIZES.map((s) => (
                  <button
                    key={s}
                    type="button"
              ...
              {(['description', 'reviews'] as const).map((t) => (
                <button
                  ...
                  className={`px-5 py-2 rounded-full text-sm font-medium capitalize transition-colors ${
                    tab === t ? 'bg-femme-champagne text-femme-black border-femme-champagne' : 'btn-filter'
                  }`}
                >
                  {t}
                </button>
            ...
            <p className="mt-4 text-arctic-deep/70 leading-relaxed text-sm normal-case tracking-normal font-normal">
              {tab === 'description'
                ? product.description || 'Premium quality piece from our curated collection. Crafted for comfort and style.'
